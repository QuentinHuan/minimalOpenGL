...
void main() {
  ...
  //  Specular Component
  //----------------------||
  float alpha = texture(material.texture_roughness1, TexCoords).r;
  float alphaSqr = pow(alpha, 2);
  float metallic = texture(material.texture_metalness1, TexCoords).r;
  ...
  vec3 AO = texture(material.texture_AO1, TexCoords).rgb;
